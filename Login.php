...
class Login
{
    ...
    {
        $query = "INSERT INTO users(id,name,email,password,address, phone,age,admin) VALUES(?,?,?,?,?,?,?,?);";
        $id = hash('sha1', $user->get_data()['name'] . $user->get_data()['email']);
        ...
        try { // Tenta conectar ao Banco de Dados
            ...
            $execution->bind_param('ssssssss', $id, $user->get_data()['name'], $user->get_data()['email'], $hashed_password, $user->get_data()['address'], $user->get_data()['phone'], $user->get_data()['age'], $user->get_data()['admin']);

            $execution->execute(); // Se a conexão for estabelecida, executa a query
            $result = $execution->affected_rows;
            $conn->close();

            Response::send(200, [$result]); // Envia a resposta ao cliente através da
